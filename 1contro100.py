...
# ----------------------------------------------------------------------------
# 1 contro 100
# Copyright © 2021-2026 Sergey Chernov aka Gamer
#
...
odpowiedzi = []
help = [True, True, True]
cost = [50, 100, 150, 200, 250, 300, 400, 500, 700, 1000]
costq = None
diff = [0.0, 2.0, 3.5]
...
jolly_active = [False, False, False]

class double_dip(Enum):
    before_ans1 = 0
    answer1_check = 1
    answer1_wrong = 2
    no = 3

jolly2 = double_dip.no
...
#
def life_hide():
    global jolly_buttons
    for i in range(3):
        jolly_buttons[i].place_forget()
#
def life_show():
    global jolly_buttons
    for i in range(3):
        jolly_buttons[i].place(x=900, y=400+45*i)
...
def raspil():
    global error_count, money_count, playerz, muyt, left_wrong, pl, baza, number_inbase, costq, number, currentq, temp_string, razd, OPV, prachy
    number = 0
    for a in range(OPV):
        if pl[a]["Active"]:
            number += 1
    ...
    if (currentq >= 1) and (number > 0):
        if mb.askyesno("Деньги или игра?", "У вас " + str(prachy) + ", и " + str(number) + ' ' + money_or_mob(
            ...
            mb.showinfo("Следующий вопрос",
                        "Стоимость вопроса - " + str(costq) + " за каждого неверно ответившего игрока.")
            show_no_of_foes()
            hide_error()
        ...
        prachy = 200000
        mb.showinfo("ДЖЕКПОТ!", "Поздравляем, вы выиграли 200000!")
        log.write("Выигрыш: 200000"+'\n')
        log.close()
        root.destroy()
    else:
        currentq += 1
...
def accepted(ipconfig):
    global stadiya, OPV, pl, tysk, playerz, otv, buchstabe, hero_answer, jolly2
    if (jolly_active[2]):
        jolly_buttons[2]["state"] = "disabled"
        if (otv[ipconfig]>0):
            ...
            mb.showinfo("Третий джокер", "Ответ " + buchstabe(ipconfig) + " выбрало игроков: "+str(otv[ipconfig])+'\n')
            for a in range(OPV):
                if(pl[a]["Active"]):
                    playerz[a]["image"]=blue
                    pl[a]['PlayerState'] = PlayerState.blue
            ...
            stadiya = q_stage.default
        help[2] = False
        jolly_active[2] = False
        life_show()
        log.write('Игрок берёт третий джокер и проверяет ответ '+buchstabe(ipconfig)+'. Игроков, выбравших данный ответ: '+str(otv[ipconfig])+'\n')
    elif (stadiya == q_stage.default):
        mb.showinfo("Ответ "+buchstabe(ipconfig), baza[number_inbase]["Variants"][ipconfig])
        if (jolly2==double_dip.no) or (( (jolly2==double_dip.before_ans1) and (ipconfig==baza[number_inbase]["Correct"])) or (jolly2==double_dip.answer1_wrong) ):
            if (jolly2 == double_dip.no):
                log.write('Ответ игрока:' + buchstabe(ipconfig) + '\n')
            elif ((jolly2==double_dip.before_ans1) and (ipconfig==baza[number_inbase]["Correct"])):
                log.write('Первый ответ:' + buchstabe(ipconfig) + '\n')
            else:
                log.write('Второй ответ:' + buchstabe(ipconfig) + '\n')
            life_hide()
        ...
        elif ((jolly2==double_dip.before_ans1) and not (ipconfig==baza[number_inbase]["Correct"])):
            life_hide()
            log.write('Первый ответ:' + buchstabe(ipconfig) + '\n')
            odpowiedzi[ipconfig]["bg"] = "#FFFF7F"
            stadiya = q_stage.accepted
            hero_answer = ipconfig
            acc()
            root.check1 = root.after(2500, lambda lok=ipconfig: doubledip_1wrong(lok))

    pass #здесь будут действия при окончательном ответе



def doubledip_1wrong(kito_kumi):
    global baza, number_inbase, odpowiedzi, tysk, pytanie, muyt, pl, ar_atsakymas_yra_teisingas, jolly2, stadiya
    root.after_cancel(root.check1)
    odpowiedzi[kito_kumi]["bg"] = "#7F7F7F"
    mb.showinfo("Ошибка!", "Вы ошиблись!")
    acc_rev()
    tysk[kito_kumi]['state'] = 'disabled'
    jolly2 = double_dip.answer1_wrong
    stadiya = q_stage.default
    life_show()
...
def jolly(ii):
    global jolly2, prachy
    if (prachy<JOLLIES_COST):
        mb.showwarning("Недостаточно денег!", "У вас меньше "+ str(JOLLIES_COST) + " на счету")
    else:
        if (((ii == 1) and (help[0] is True)) or ((ii == 2) and ((help[0] is True) or (help[1] is True)))):
            mb.showwarning("Соблюдайте очерёдность", "Не использованы предыдущие Джокеры")
        else:
            prachy -= JOLLIES_COST
            if (ii == 0):
                jolly_buttons[0]["state"] = "disabled"
                life_hide()
                log.write("Игрок использует первый джокер, позволяющий пропустить вопрос."+'\n')
                help[0] = False
                accepted(baza[number_inbase]["Correct"])
            elif (ii == 1):
                jolly_buttons[1]["state"] = "disabled"
                log.write("Игрок использует второй джокер, позволяющий дать второй ответ, если первый неверен." + '\n')
                jolly_active[1] = True
                help[1] = False
                jolly2 = double_dip.before_ans1
                # дописать
            elif(ii == 2):
                jolly_buttons[2]["state"] = "disabled"
                # log.write("Игрок использует второй джокер, позволяющий дать второй ответ, если первый неверен." + '\n')
                jolly_active[2] = True
                help[2] = False
                mb.showinfo("Третий джокер", "Выберите ответ, который хотите проверить")




def sh_q_variants():
    global pytanie, odpowiedzi, kola_ratunkowe, number_inbase, ya, vin, poll_the_mob, ask_the_mob, trust_the_mob, help, tysk, jolly2, vyluchenyi, ar_atsakymas_yra_teisingas, log, currentq, costq, number
    ...
    tysk = []
    odpowiedzi = []
    log.write('Вопрос ' + str(currentq) + ' (' + str(costq) + ')'+'\n')
...
def pradeti():
    ...
    if (currentq<(len(cost))):
        ...
        costq = cost[-1]
    mb.showinfo("Следующий вопрос", "Стоимость вопроса - "+str(costq)+" за каждого неверно ответившего игрока.")
...
for i in range(3):
    jb = tk.Button(root, state="normal", text="Джокер "+str(i+1), height=1, width=9, command=lambda i=i: jolly(i))
    jolly_buttons.append(jb)
del jb
